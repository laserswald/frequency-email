...
class TabbedToolbar(Notebook):
	...
	def add_toolframe(self, page):
		...
		toolframe.pack(side = 'left', fill = 'x', expand = 1)
		return toolframe
...
class MainButton(Menubutton):
	...
	def add_command(self, label, command, icon=None, compound=LEFT):
		'''
...
class Ribbon(Frame):
	...
	def __init__ (self, parent):
		...
		self.toolbar.grid(row = 0, column = 1, sticky = E+W)
...
if __name__ == '__main__':
	app = Tk()
	rib = Ribbon(app)
	testingpage = rib.toolbar.add_page('Testing')
	toolbar = rib.toolbar.add_toolframe(testingpage)
	toolbar.insert('Test button', None, None, None)
